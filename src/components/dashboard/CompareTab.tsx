    ...
    <div className="space-y-4 animate-fade-in">
      <div className="dashboard-panel p-4">
        ...
        <div className="flex flex-wrap gap-3">
          <select value={distA} onChange={e => setDistA(e.target.value)} className="h-9 px-3 text-sm rounded-lg bg-secondary border border-border text-foreground">
          ...
          <select value={distB} onChange={e => setDistB(e.target.value)} className="h-9 px-3 text-sm rounded-lg bg-secondary border border-border text-foreground">
              ...
              return (
                <div key={i} className="kpi-card">
                  ...
                  <div className="flex justify-between mt-1">
                    <div>
                      <div className="text-[10px] text-muted-foreground">{a.DIS_NAME}</div>
                      <div className="text-lg font-bold text-primary">{item.A}{item.unit}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[10px] text-muted-foreground">{b.DIS_NAME}</div>
                      <div className="text-lg font-bold text-accent">{item.B}{item.unit}</div>
                    </div>
                  </div>
                  <div className={`flex items-center gap-1 mt-1 text-[10px] font-semibold ${colorA}`}>
                    ...
                    <span>
                      {diff.pct > 0 ? `${a.DIS_NAME} is ${diff.pct.toFixed(0)}% ${diff.direction}` : 'Equal'}
                    </span>
                  </div>
                </div>
              );
